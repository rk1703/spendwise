...
// Optimized imports: import only required icons to enable tree-shaking
import {
  Utensils, Car, Home, ShoppingBag, FileText, HeartPulse, Ticket, Briefcase,
  GraduationCap, Gift, TrendingUp, Tags, Coffee, Beer, ShoppingCart, Plane,
  Bike, Bus, Dumbbell, Activity, Heart, Calendar, Camera, Landmark, DollarSign,
  CreditCard, PiggyBank, Sparkles, HelpCircle, Lock, User, Users, Globe, Film,
  Music, Tv, Gamepad2, Book, Phone, Laptop, Shirt, Trophy, Dog, Cat, Wine,
  Scissors, MapPin, Hammer, Wrench, Smile, Moon, Sun, Cloud, Tag, PlusCircle,
  Edit3, Trash2, Loader2, type LucideIcon
} from 'lucide-react';

// Curated finance-centric icons dictionary
export const categoryIcons = {
  ...
  CreditCard, PiggyBank, Sparkles, HelpCircle, Lock, User, Users, Globe, Film,
  Music, Tv, Gamepad2, Book, Phone, Laptop, Shirt, Trophy, Dog, Cat, Wine,
  Scissors, MapPin, Hammer, Wrench, Smile, Moon, Sun, Cloud, Tag
};

const iconKeys = Object.keys(categoryIcons) as (keyof typeof categoryIcons)[];
...
// Helper to generate a random HSL color once on mount/initialization
...
const CategoryForm = ({ category, onSave, onCancel }: { category?: Category, onSave: () => void, onCancel: () => void }) => {
  ...
  // Generate random color only once per form initialization
  const initialColor = useMemo(() => category?.color || getRandomColor(), [category]);
  ...
  // Watch the active icon field to display the preview reactively
  const watchIcon = watch('icon', 'Tag');
  const SelectedIconPreview = categoryIcons[watchIcon as keyof typeof categoryIcons] || Tag;

  const onSubmit = async (data: CategoryFormData) => {
    const categoryNameLower = data.name.toLowerCase();
    
    // Check duplication locally
    const isDuplicate = categories.some(
      c => c.name.toLowerCase() === categoryNameLower && c.id !== category?.id
    );

    if (isDuplicate) {
      setValue('name', data.name, { shouldTouch: true });
                ...
                <ScrollArea className="h-[200px]">
                  <div className="p-2 grid grid-cols-5 gap-1">
                    {iconKeys.map(iconKey => {
                      const Icon = categoryIcons[iconKey] || Tag;
                      return (
                        <Button
                          key={iconKey}
                          type="button"
    ...
    <Card className="shadow-lg">
      <CardHeader className="flex flex-col gap-2 md:flex-row items-center justify-between">
        <div>
          ...
          <CardDescription className='tracking-tight md:tracking-normal'>
            Add, edit, or delete your spending categories.
          </CardDescription>
        </div>
      ...
      <CardContent>
        {categories.length > 0 ? (
          <ul className="space-y-3">
            {categories.map((cat) => {
              const IconComponent = categoryIcons[cat.icon as keyof typeof categoryIcons] || Tag;
              return (
